...
import com.kaige.distribution.transaction.order.entity.OrderPaidEvent;
import com.kaige.distribution.transaction.order.entity.ProductWithPayload;
import com.kaige.distribution.transaction.order.entity.User;
...
public class RocketMqConfig {
  ...
  @Value("${my.rocketmq.transTopic}")
  private String springTransTopic;

  @Value("${my.rocketmq.topic}")
  private String springTopic;

  @Value("${my.rocketmq.topicUser}")
  private String userTopic;

  @Value("${my.rocketmq.orderTopic}")
  private String orderPaidTopic;

  @Value("${my.rocketmq.msgExtTopic}")
  private String msgExtTopic;

  @Value("${my.rocketmq.stringRequestTopic}")
  private String stringRequestTopic;

  @Value("${my.rocketmq.bytesRequestTopic}")
  private String bytesRequestTopic;

  @Value("${my.rocketmq.objectRequestTopic}")
  private String objectRequestTopic;

  @Value("${my.rocketmq.genericRequestTopic}")
  ...
  public void run(String... args) throws Exception {
    ...
    sendResult =
        rocketMQTemplate.syncSend(
            userTopic, User.builder().userAge((byte) 18).userName("Kitty").build());
    System.out.printf("syncSend1 to topic %s sendResult=%s %n", userTopic, sendResult);

    sendResult =
        rocketMQTemplate.syncSend(
            userTopic,
            MessageBuilder.withPayload(User.builder().userAge((byte) 21).userName("Lester").build())
    ...
    sendResult =
        extRocketMQTemplate.syncSend(
            springTopic, MessageBuilder.withPayload("Hello, World!2222".getBytes()).build());
    System.out.printf(
        "extRocketMQTemplate.syncSend1 to topic %s sendResult=%s %n", springTopic, sendResult);
    ...
    rocketMQTemplate.convertAndSend(
        msgExtTopic + ":tag0", "I'm from tag0"); // tag0 will not be consumer-selected
    ...
    String replyString =
        rocketMQTemplate.sendAndReceive(stringRequestTopic, "request string", String.class);
    ...
    byte[] replyBytes =
        rocketMQTemplate.sendAndReceive(
            bytesRequestTopic,
            MessageBuilder.withPayload("request byte[]").build(),
            byte[].class,
            3000);
    ...
    User requestUser = User.builder().userAge((byte) 9).userName("requestUserName").build();
    User replyUser =
        rocketMQTemplate.sendAndReceive(objectRequestTopic, requestUser, User.class, "order-id");
    System.out.printf("send %s and receive %s %n", requestUser, replyUser);
    // Send request in sync mode with timeout and delayLevel parameter parameter and receive a reply
    // of generic type.
    ProductWithPayload<String> replyGenericObject =
        rocketMQTemplate.sendAndReceive(
            genericRequestTopic,
            "request generic",
            new ParameterizedTypeReference<ProductWithPayload<String>>() {}.getType(),
            30000,
            2);
    ...
    rocketMQTemplate.sendAndReceive(
        objectRequestTopic,
        User.builder().userAge((byte) 9).userName("requestUserName").build(),
  ...
  private void testBatchMessages() {
    ...
    for (int i = 0; i < 10; i++) {
      msgs.add(
          MessageBuilder.withPayload("Hello RocketMQ Batch Msg#" + i)
  ...
  private void testSendBatchMessageOrderly() {
    for (int q = 0; q < 4; q++) {
      ...
      System.out.println(
          "--- Batch messages orderly to queue :"
  ...
  private void testRocketMQTemplateTransaction() throws MessagingException {
    ...
    for (int i = 0; i < 10; i++) {
      try {
        ...
        Message msg =
            MessageBuilder.withPayload("rocketMQTemplate transactional message " + i)
        ...
        System.out.printf(
            "------rocketMQTemplate send Transactional msg body = %s , sendResult=%s %n",
  ...
  private void testExtRocketMQTemplateTransaction() throws MessagingException {
    for (int i = 0; i < 10; i++) {
      try {
        Message msg =
            MessageBuilder.withPayload("extRocketMQTemplate transactional message " + i)
                .setHeader(RocketMQHeaders.TRANSACTION_ID, "KEY_" + i)
                .build();
        SendResult sendResult =
            extRocketMQTemplate.sendMessageInTransaction(springTransTopic, msg, null);
        System.out.printf(
            "------ExtRocketMQTemplate send Transactional msg body = %s , sendResult=%s %n",
            msg.getPayload(), sendResult.getSendStatus());
  ...
  class TransactionListenerImpl implements RocketMQLocalTransactionListener {
    ...
    private final ConcurrentHashMap<String, Integer> localTrans =
    ...
    public RocketMQLocalTransactionState executeLocalTransaction(Message msg, Object arg) {
      ...
      System.out.printf(
          "#### executeLocalTransaction is executed, msgTransactionId=%s %n", transId);
  ...
  class ExtTransactionListenerImpl implements RocketMQLocalTransactionListener {
    ...
    public RocketMQLocalTransactionState executeLocalTransaction(Message msg, Object arg) {
      System.out.printf(
          "ExtTransactionListenerImpl executeLocalTransaction and return UNKNOWN. \n");
      return RocketMQLocalTransactionState.UNKNOWN;
    }
