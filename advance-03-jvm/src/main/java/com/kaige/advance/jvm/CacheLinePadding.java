 ...
 * 验证更新缓存行时引发的性能问题
 *
 * <p>Intel CPU 的缓存行默认大小是 64byte， 所以我们构建两个数组，一个数组大小小于 64byte，处于一个缓存行中； 第二个数组大于
 * 64byte，操作的数据不在同一个缓存行中； 执行程序比较两者计算耗时
 */
public class CacheLinePadding {
  ...
  /**
   * JDK8 之后，通过使用加上 @Contended 注解，可以保证字段处于不同的缓存行中，jvm 默认是开启，即允许竞争，可以通过增加 -XX:-RestrictContended
   * 参数关闭禁止竞争
   */
  // @Contended
