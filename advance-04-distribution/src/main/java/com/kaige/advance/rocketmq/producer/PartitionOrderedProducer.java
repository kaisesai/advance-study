 ...
 * 分区有序生产者
 *
 * <p>消息有序指的是可以按照消息的发送顺序来消费(FIFO)。RocketMQ可以严格的保证消息有序，可以分为分区有序或者全局有序。
 *
 * <p>顺序消费的原理解析，在默认的情况下消息发送会采取Round
 ...
 *
 * <p>下面用订单进行分区有序的示例。一个订单的顺序流程是：创建、付款、推送、完成。订单号相同的消息会被先后发送到同一个队列中，消费时，同一个OrderId获取到的肯定是同一个队列。
 */
public class PartitionOrderedProducer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // 订单列表
    List<OrderStep> orderList = new PartitionOrderedProducer().buildOrders();
  ...
  private List<OrderStep> buildOrders() {
    ...
    long orderId1 = 15103111039L;
    long orderId2 = 15103111065L;
    long orderId3 = 15103117235L;

    OrderStep orderDemo = new OrderStep();
    orderDemo.setOrderId(orderId1);
    orderDemo.setDesc("创建");
    orderList.add(orderDemo);

    orderDemo = new OrderStep();
    orderDemo.setOrderId(orderId2);
    orderDemo.setDesc("创建");
    orderList.add(orderDemo);
